...
// Validation schema for feedback response
const FeedbackResponseSchema = z.object({
  feedbackId: z.string(),
  response: z.string().min(1, "Response cannot be empty"),
  status: z.enum(['open', 'in_progress', 'planned', 'resolved', 'closed']).optional()
});

// Validation schema for feedback status update
const FeedbackStatusSchema = z.object({
  feedbackId: z.string(),
  status: z.enum(['open', 'in_progress', 'planned', 'resolved', 'closed'])
});
...
export async function GET(request: NextRequest) {
  try {
    ...
    // and only return feedback for apps they own
    // For now, we'll just filter by appId if provided
    
    if (appId && appId !== 'all') {
      filteredFeedback = filteredFeedback.filter(item => item.appId === appId);
    }
    ...
    if (rating && rating !== 'all') {
      filteredFeedback = filteredFeedback.filter(item => item.rating === parseInt(rating, 10));
    }
    ...
    if (timeRange) {
      ...
      filteredFeedback = filteredFeedback.filter(item => {
        ...
        return feedbackDate >= startDate && feedbackDate <= currentDate;
      });
    }
...
// POST /api/feedback (for responding to feedback)
export async function POST(request: NextRequest) {
  try {
    ...
    const feedbackIndex = feedbackDatabase.findIndex(item => item.id === validatedData.feedbackId);
    
    if (feedbackIndex === -1) {
      return NextResponse.json({ error: 'Feedback not found' }, { status: 404 });
    }
    
    // Update the feedback with developer response
    const updatedFeedback = {
      ...feedbackDatabase[feedbackIndex],
      developerResponse: validatedData.response,
      updatedAt: new Date().toISOString(),
      // Update status if provided, otherwise if status is 'open', change to 'in_progress'
      status: validatedData.status || 
        (feedbackDatabase[feedbackIndex].status === 'open' ? 'in_progress' : feedbackDatabase[feedbackIndex].status)
    };
    
    // Update the database
    feedbackDatabase[feedbackIndex] = updatedFeedback;
    ...
    return NextResponse.json({ error: 'Failed to respond to feedback' }, { status: 500 });
  }
}

// PATCH /api/feedback (for updating feedback status)
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Parse and validate request body
    const body = await request.json();
    const validatedData = FeedbackStatusSchema.parse(body);
